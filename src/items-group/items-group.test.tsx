  ...
  },
  {
    ...
    svg: (
      <svg
        ...
        viewBox="0 0 100 100"
      >
        <path d="M39.1 77l48.7-48.7c1.5-1.4 1.6-3.8.2-5.3-1.4-1.5-3.8-1.6-5.3-.2l-.2.2-46.1 46.1-19.8-19.9c-1.5-1.4-3.9-1.3-5.3.2-1.3 1.4-1.3 3.7 0 5.1L33.8 77c1.4 1.5 3.8 1.5 5.3 0z"></path>
      </svg>
    ),
    value: 'second',
  },
...
describe('Items group', () => {
  it('should be rendered without crashing', async () => {
    const { container } = render(
      <ItemsGroup id="test-items-group" items={items} value="first" />
    );

    expect(container.querySelector('#test-items-group')).toBeInTheDocument();
  });

  it('should add active state for the item that is selected', async () => {
    const { container } = render(
      <ItemsGroup id="test-items-group" items={items} value="first" />
    );

    expect(
      container.querySelector('#test-items-group button:first-child')
    ).toHaveClass('active');
  });
});
