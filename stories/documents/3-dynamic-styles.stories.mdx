...
FlatifyCSS itself has its stylesheet and is written in pure CSS with meaningful class names. However, when it comes to react and a real project, styled components is just a great help to fill the gap between that stylesheet and react components.
...
This button has styles that are conditionally set based on `clicked` state. Styles are set like strings, and CSS properties are given by FlatifyCSS to customize the `.button` class styles.
